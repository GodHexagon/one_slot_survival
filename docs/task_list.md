...
- プレイヤー有効性設定の保存に失敗したとき、ミュートされている：com/github/godhexagon/oneslotsurvival/OneSlotManager.java:71
- ゲームモード変更時のイベントがあるので、それを使えば[こういう](com/github/godhexagon/oneslotsurvival/OneSlotEvents.java:37)実装にしなくていい
- 全体的にクライアント専用とサーバー専用がわかりにくい
- 全ワールドで共通のプレイヤー設定になっている
- 多分、マルチプレイヤーのときに、ログアウトしているときに有効にされると変なことになる
