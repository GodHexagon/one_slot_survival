...
#### 8. 実装が必要な最小限の箇所（必須）
1. **サーバー側管理**: プレイヤーごとのMOD有効性を管理するクラス
2. **NBT永続化**: プレイヤーデータに保存・読み込み
3. **ネットワーク同期**: クライアントに状態を送信
4. **能力制御**: MOD有効性に応じた制限を適用

#### 9. 実装が推奨される箇所
1. **コマンド**: 管理者が変更できるコマンド
2. **Forgeイベント**: 他MODとの連携
3. **クライアントUI**: 状態の可視化

#### 10. 実装不要な箇所
- **ワールドデフォルト設定**: プレイヤーごとの設定なので不要
- **進捗システム**: MOD有効性は進捗条件にしない想定
- **Realms統合**: Realms対応は後回し
- **テレメトリー**: プライバシー観点で不要
