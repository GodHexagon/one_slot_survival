...
public class SubRoleId {
    ...
    /**
     * 指定されたプレイヤーのサブロールIDを取得
     * クライアントとサーバーの両側で動作 - 自動的に同期される
     *
     * @param player チェックするプレイヤー
     * @return プレイヤーのサブロールID、未割り当ての場合は UNASSIGNED (0)
     * @throws IllegalStateException SUB_ROLE_ID Attribute が登録されていない場合
     */
    ...
    /**
     * 指定されたプレイヤーにサブロールIDを設定
     * 自動的に永続化され、クライアントと同期される
     *
     * @param player 変更するプレイヤー
     * @param roleId 設定するロールID（0 の場合は未割り当て）
     * @throws IllegalStateException SUB_ROLE_ID Attribute が登録されていないか、プレイヤーに存在しない場合
     */
    public static void setRoleId(ServerPlayer player, int roleId) {
        AttributeInstance attribute = player.getAttribute(getAttributeHolder());
        if (attribute == null) {
            throw new IllegalStateException(
                    "Player does not have SUB_ROLE_ID attribute. " +
